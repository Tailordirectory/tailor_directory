...
// сuts one array on many arrays(chunks) by fixed element length
...
class Email {
  ...
  async sendEmailWithTemplate({
    ...
    return this.getClient().sendEmailWithTemplate({
      From,
      TemplateId,
      To,
      TemplateModel,
  ...
  async sendEmailBatchWithTemplates(messages) {
    ...
    const promises = chunks.map(async (chunk) =>
      this.getClient().sendEmailBatchWithTemplates(chunk)
    );

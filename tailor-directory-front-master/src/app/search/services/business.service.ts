...
export class BusinessService {
  ...
  getBusinessById(id: string): Observable<BusinessModel | null> {
    return this.http.get<BusinessModel>(this.apiService.SEARCH_TAILORS + '/' + id).pipe(map(r => {
      if (r) {
        const basic = 'basic';
  ...
  getOtherOwnerBusinessList(businessId: string): Observable<BusinessModel[]> {
    return this.http.get<BusinessModel[]>(this.apiService.SEARCH_TAILORS + '/another/' + businessId);
  }

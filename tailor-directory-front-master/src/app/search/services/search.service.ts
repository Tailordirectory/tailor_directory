...
export class SearchService {
  ...
  findTailors(address: AddressSearchModel, filter: SearchFilterModel, geolocation: GeocodingSearchModel, page?: number, limit?: number): Observable<BusinessResponseModel | null> {
    const params = {
    ...
    return this.http.get<BusinessResponseModel>(this.apiService.SEARCH_TAILORS, {
      params
    }).pipe(map(result => {
  ...
  getFullTailorsList(): Observable<BusinessModel[]> {
    return this.http.get<BusinessModel[]>(this.apiService.SEARCH_TAILORS);
  }

  getBusinessTypeList(): Observable<string[]> {
    return this.http.get<string[]>(this.apiService.SEARCH_TAILORS + '/types');
  }

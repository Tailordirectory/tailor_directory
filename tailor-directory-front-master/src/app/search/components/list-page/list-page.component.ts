...
export class ListPageComponent implements OnDestroy, AfterViewInit {
        ...
        if (p.lt && p.ln) {
          ...
          if (this.searchForm.get('address')?.value === '') {
            this.findByCoordinates((result: google.maps.GeocoderResult) => {
              ...
              this.getTailorsList(address, filter, {lat: p.lt, long: p.ln});
            });
          } else {
            const place: google.maps.places.PlaceResult = this.autoComplete.getPlace();
            if (place) {
  ...
  private getTailorsList(address: AddressSearchModel, filter: SearchFilterModel, geolocation: GeocodingSearchModel) {
    const pageCount = this.searchForm.get('pageCount')?.value;
    this.searchService.findTailors(address, filter, geolocation, this.page, parseInt(pageCount, 10)).pipe(tap(r => {
      this.results = '';
      if (r) {
        this.pageCount = pageCount;
        this.total = r.total;

...
@Component({
  selector: 'app-edit-business-profile-form',
...
export class EditBusinessProfileFormComponent implements AfterViewInit, OnDestroy {
  ...
  constructor(
    ...
    this.$resetUpload = new Subject<void>();
  ...
  getBusiness() {
    if (this.business) {
      ...
      this.serviceForm.get('tailorsCount')?.setValue(business.numberOfTailors);
  ...
  private saveProfile() {
    ...
    if (servicesForm.tailorsCount) {
      Object.assign(business, {numberOfTailors: servicesForm.tailorsCount});
    }
    ...
    if (this.isEdit && this.business) {
      this.businessService.updateBusiness(this.business._id, business).subscribe(result => {
      ...
      this.businessService.addNewBusiness(business).subscribe(r => {
        if (this.redirectToPreview) {
          this.router.navigate(['/profile', r._id]);
          this.onSuccess.emit();
        } else {
  ...
  onLogoUpload(event: any) {
    ...
    this.businessService.uploadLogo(this.business?._id, this.logoFormData).pipe(takeUntil(this.$resetUpload))
      .subscribe(
        (resp) => {
          if (this.business) {
            this.business.icon = resp.doc;
          }
        });
  }
  ...
  onGalleryUpload(event: any) {
    ...
    files.forEach(f => {
      this.galleryFormData.append('files', f);
    });
    this.businessService.uploadMedia(this.business?._id, this.galleryFormData).subscribe(result => {
      if (this.business) {
        this.business.media = result.doc;
      }
    });
  }
  ...
  ngAfterViewInit(): void {
    ...
    if (this.isEdit) {
      setTimeout(() => {
        ...
        this.galleryRef = this.getElementById('upload-gallery-input');
      });
    }
  ...
  onUploadGallery() {
    this.galleryRef.click();
  }

  onDeletePhoto(index: number) {
    if (this.business) {
      const media = this.business.media[index];
      const ids = [media._id, media._id];
      this.businessService.deleteMedia(this.business._id as string, ids).subscribe(r => {
        this.business?.media.splice(index, 1);
      });
    }
  }
  ...
  private socialMediaValidators(regExp: RegExp): ValidatorFn {
    return (control: AbstractControl): { [key: string]: any } | null => {
      if (control && control.value) {
        ...
        return null;
        // const forbidden = regExp.test(val);
        // return !forbidden ? {socilaValidator: true} : null;
      } else {
        return null;
      }
    };

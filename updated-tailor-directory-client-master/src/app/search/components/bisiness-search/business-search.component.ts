...
import * as fromBusinessSearchActions from '../../storage/actions/search.actions';
...
export class BusinessSearchComponent implements AfterViewInit, OnDestroy {
  ...
  private onSuccessSearch(place: google.maps.GeocoderResult | google.maps.places.PlaceResult) {
    if (place && place.geometry && place.formatted_address) {
      ...
      this.searchStore.dispatch(fromBusinessSearchActions.setSearchDataAction({
        search: {
  ...
  private getSearchData(direction: GeocodingSearchModel) {
    ...
    this.searchStore.dispatch(fromBusinessSearchActions.loadResultsActions());
  }
  ...
  ngAfterViewInit(): void {
    ...
    this.searchStore.pipe(select(searchBusinessListSelectors.businessListSelector)).pipe(takeUntil(this.unsubscribe)).subscribe(listState => {

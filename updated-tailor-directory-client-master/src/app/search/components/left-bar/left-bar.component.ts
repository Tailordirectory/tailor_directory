...
export class LeftBarComponent implements AfterViewInit, OnDestroy {
  ...
  ngAfterViewInit(): void {
    this.searchStore.pipe(select(searchBusinessListSelectors.businessListSelector)).pipe(takeUntil(this.unsubscribe)).subscribe(result => {
      if (result.isLoaded) {
        this.isHidden = false;
        if (result.businessList.length > 0) {
          this.isOpened = true;
          this.searchStore.dispatch(leftPanelAction.showPanelAction());
        }
      }

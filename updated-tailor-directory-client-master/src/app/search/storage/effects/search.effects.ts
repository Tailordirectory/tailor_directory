...
import * as businessListActions from '../actions/businessList.actions';
...
export class SearchEffects {
  ...
  $loadBusiness = createEffect(() => this.actions.pipe(
    ...
    switchMap(() => {
      return this.businessListState.select(fromSearchSelectors.getSearchSelector).pipe(switchMap(d => {
        ...
        return this.searchService.findBusiness(address, filter, geolocation).pipe(map((result) => {
          this.businessProfileState.dispatch(businessProfileActions.hideUserProfileStateAction());
          return (businessListActions.loadedBusinessList({list: (result) ? result.docs : []}));
        }));
      }));
    })
  ));
